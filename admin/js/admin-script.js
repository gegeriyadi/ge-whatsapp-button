...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('#ge-generate-link').on('click', function() {
            const message = $('#ge-custom-message').val().trim();
            ...
            if (!rotatorUrl) {
                alert('Please configure the Rotator Service URL first');
                return;
            }
            
            // Generate links
            const encodedMessage = encodeURIComponent(message);
            const directLink = rotatorUrl + '?message=' + encodedMessage;
            const shortcode = `[ge_whatsapp_button message="${message.replace(/"/g, '&quot;')}"]`;
            const htmlCode = `<a href="${directLink}" class="ge-wa-custom-link" target="_blank">Chat WhatsApp</a>`;
            
            // Display results
            $('#ge-direct-link').text(directLink);
            $('#ge-shortcode').text(shortcode);
            $('#ge-html-code').text(htmlCode);
